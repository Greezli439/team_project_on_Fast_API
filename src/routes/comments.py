...
async def create_comment(body: CommentModel, current_user: User = Depends(auth_service.get_current_user),
...
async def remove_comment(comment_id: int = Path(ge=1), current_user: User = Depends(auth_service.get_current_user),
                         db: Session = Depends(get_db)):
